...
class PythonTask(object):
    ...
    def compose_query(self,task_spec,batch_mode):
        ...
        if not task_spec['query']['custom'] and len(task_spec['table']) == 1:
            ...
            _cdc = []
            interval_keywords = {'day','hour','minute','second'}
            for key,val in task_spec.get('table',{})[0].get('cdc',{}).items():
                ...
                if isinstance(val,str) and any(elem in val for elem in interval_keywords):
                    ...
                    interval = re.search(pattern, val).group()
            ...
            if not batch_mode:
                query = """SELECT {columns} FROM {table_name} {where_clause} ;""".format(columns=columns,table_name=table_name,where_clause=where_clause)
            else:
                query = """SELECT {columns} FROM {table_name} {where_clause} ORDER BY {_order} LIMIT {offset},{limit} ;""".format(columns=columns,table_name=table_name,where_clause=where_clause,_order=_order,offset='{offset}',limit='{limit}')
    ...
    def get_batch_size(self,connection,**kwargs):
        ...
        GET_NO_OF_ROWS_QUERY = """SELECT table_rows FROM information_schema.tables WHERE table_schema = '{dbname}' and table_name = '{table}';"""
    ...
    def extract_data(self,**kwargs):
        ...
        config_tz = kwargs['timezone']
        now_utc = datetime.datetime.now(pytz.utc)
        timestamp_suffix = now_utc.strftime('%Y%m%d%H%M%S')

        with MySQLDBConnection(host=kwargs['host'], user=kwargs['user'], password=kwargs['password'], db=kwargs['db']) as connection:

            row_batch_size = self.get_batch_size(connection,dbname=kwargs['db'],tables=[table['name'] for table in kwargs['task_spec']['table']], batch_threshold=kwargs['batch_threshold'])
            ...
            if not batch_mode:
                ...
                while True:
                    ...
                    if data:
                        streamed_file = stream_to_s3(kwargs['task_spec']['task_name'],data,header,timestamp_suffix,batch_id=batch_id)
                        print(streamed_file,'uploaded')
